...
class MoEConfig8x7B(MoEConfig):
    ...
    ffn_hidden_size: int = 7168
    max_position_embeddings: int = 4096
    seq_length: int = 4096


@dataclass
class MoEConfig8x22B(MoEConfig):
    """
...
class MoEModel(GPTModel):
    def __init__(
        self,
        config: Optional[Union[MoEConfig8x3B, MoEConfig8x22B]] = None,
        optim: Optional[OptimizerModule] = None,
        tokenizer: Optional["TokenizerSpec"] = None,
        model_transform: Optional[Callable[[nn.Module], nn.Module]] = None,
    ):
        super().__init__(
            config or MoEConfig8x7B(), optim=optim, tokenizer=tokenizer, model_transform=model_transform
        )


@io.model_importer(MoEModel, ext="hf")
class HFMixtralImporter(io.ModelConnector["MixtralForCausalLM", MoEModel]):
    def init(self) -> MoEModel:
        return MoEModel(self.config, tokenizer=self.tokenizer)
    ...
    @property
    def config(self) -> MoEConfig8x7B | MoEConfig8x22B:
        from transformers import MoEConfig as HfMixtralConfig

        config = HfMixtralConfig.from_pretrained(str(self))
        config_cls = MoEConfig8x7B
        if '8x22b' in str(self).lower():
            config_cls = MoEConfig8x22B
        return config_cls(
...
@io.model_exporter(MoEModel, "hf")
class HFMixtralExporter(io.ModelConnector[MoEModel, "MixtralForCausalLM"]):
    ...
    def config(self) -> "MoEConfig":
        # Either MoEConfig8x7B or MoEConfig8x22B
        source: MoEConfig8x7B = io.load_ckpt(str(self)).model.config
